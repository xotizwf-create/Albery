...
from utils import (  # noqa: F401 — re-exported; moved verbatim out of app.py 2026-07-02
    ...
    to_int,    RU_MONTH_NAMES,
    first_text_value,
    safe_parse_date,
    sentence_case_ru,
)
from llm import (  # noqa: F401 — re-exported; moved verbatim out of app.py 2026-07-02
    _google_ocr_model_candidates,
    _parse_model_list,
    _retry_after_seconds,
    is_google_genai_base,
    llm_api_base_url,
    llm_api_key,
    llm_api_url,
    llm_auth_headers,
    llm_model_for_request,
    llm_post_with_retry,
    llm_provider_name,
)
from config import (  # noqa: F401 — re-exported; moved verbatim out of app.py 2026-07-02
    APP_ROOT,
    ATTACHMENTS_DIR,
    DATABASE_URL,
    EXPORT_DIR,
    ...
    LOCAL_TZ,
    MSK_TZ,
    can_finalize_chat_day,
    chat_day_finalization_error,
    is_future_day,
    latest_visible_report_date,
    msk_now,
    msk_today,
    previous_week_bounds_for_report,
)
...
def webapp_design_template(title: str | None = None) -> dict[str, Any]:
    """Albery-branded web-app template (matches the prod React site): light bg #f6f8fb, white rounded
    cards, primary purple #5440F6, Inter font, soft shadows. Returns ready HTML with placeholders
    {{TITLE}} {{CONTENT}} {{APPLET}} + the CSS and a working content example. Use for EVERY web app."""
    css = ':root{--bg:#f6f8fb;--surface:#fff;--surface2:#f8fafc;--border:#eef0f4;--border2:#e2e8f0;\n--primary:#5440f6;--primaryD:#4532db;--primary50:#eef2ff;--primary100:#e0e7ff;\n--text:#0f172a;--muted:#64748b;--muted2:#94a3b8;--success:#16a34a;--successBg:#e7f8ed;\n--danger:#ef4444;--dangerBg:#fef2f2;--warnBg:#fef3c7;--warn:#b45309;\n--radius:14px;--radiusLg:18px;--radiusSm:10px;\n--shadow:0 1px 2px rgba(16,24,40,.04),0 10px 30px rgba(16,24,40,.06);\n--ring:0 0 0 3px rgba(84,64,246,.12),0 0 0 1px rgba(84,64,246,.36)}\n*{box-sizing:border-box}\nbody{margin:0;background:var(--bg);color:var(--text);\nfont-family:Inter,system-ui,-apple-system,"Segoe UI",Roboto,"Helvetica Neue",Arial,sans-serif;\n-webkit-font-smoothing:antialiased;text-rendering:geometricPrecision}\n.app{max-width:980px;margin:0 auto;padding:24px 16px 72px}\n.topbar{display:flex;align-items:center;gap:12px;margin-bottom:20px}\n.brand{display:flex;align-items:center;gap:10px;font-weight:800;font-size:19px;letter-spacing:-.01em}\n.brand .logo{width:32px;height:32px;border-radius:10px;background:linear-gradient(135deg,#5440f6,#8b7bff);\ndisplay:grid;place-items:center;color:#fff;font-weight:800;box-shadow:0 4px 12px rgba(84,64,246,.35)}\n.sub{color:var(--muted);font-size:13px;margin-top:2px}\n.card{background:var(--surface);border:1px solid var(--border);border-radius:var(--radiusLg);\nbox-shadow:var(--shadow);padding:22px;margin-bottom:16px}\n.card h2{margin:0 0 4px;font-size:16px;font-weight:700}\n.card .hint{color:var(--muted);font-size:13px;margin:0 0 16px}\n.btn{appearance:none;border:1px solid var(--border2);background:#fff;color:var(--text);font:inherit;\nfont-weight:600;padding:11px 18px;border-radius:12px;cursor:pointer;transition:.15s;display:inline-flex;\nalign-items:center;gap:8px}\n.btn:hover{background:var(--surface2)}\n.btn:active{transform:translateY(1px)}\n.btn-primary{background:var(--primary);border-color:var(--primary);color:#fff;\nbox-shadow:0 6px 16px rgba(84,64,246,.30)}\n.btn-primary:hover{background:var(--primaryD)}\n.btn:focus-visible{box-shadow:var(--ring)}\n.field{display:flex;flex-direction:column;gap:6px;margin-bottom:14px}\n.field label{font-size:13px;font-weight:600;color:var(--muted)}\n.input,select,textarea{font:inherit;padding:11px 13px;border:1px solid var(--border2);border-radius:12px;\nbackground:#fff;color:var(--text);width:100%;transition:.15s}\n.input:focus,select:focus,textarea:focus{outline:none;box-shadow:var(--ring);border-color:var(--primary)}\n.row{display:flex;gap:14px;flex-wrap:wrap}.row>.field{flex:1;min-width:170px}\ntable{width:100%;border-collapse:collapse;font-size:14px}\nthead th{text-align:left;font-weight:600;color:var(--muted2);padding:10px 12px;\nborder-bottom:1px solid var(--border);font-size:11px;text-transform:uppercase;letter-spacing:.04em}\ntbody td{padding:13px 12px;border-bottom:1px solid var(--border)}\ntbody tr:last-child td{border-bottom:0}\ntbody tr:hover td{background:var(--surface2)}\n.badge{display:inline-flex;align-items:center;gap:6px;padding:4px 11px;border-radius:999px;\nfont-size:12px;font-weight:600}\n.badge-success{background:var(--successBg);color:var(--success)}\n.badge-muted{background:var(--surface2);color:var(--muted)}\n.badge-warn{background:var(--warnBg);color:var(--warn)}\n.stat{display:flex;flex-direction:column;gap:4px}\n.stat .v{font-size:26px;font-weight:800;letter-spacing:-.02em}\n.stat .l{color:var(--muted);font-size:13px}\n.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:14px}\n.muted{color:var(--muted)}.right{text-align:right}.mt{margin-top:14px}\n.toast{position:fixed;left:50%;bottom:22px;transform:translateX(-50%) translateY(10px);background:#0f172a;\ncolor:#fff;padding:12px 20px;border-radius:12px;box-shadow:var(--shadow);font-size:14px;opacity:0;\ntransition:.25s;pointer-events:none;z-index:50}\n.toast.show{opacity:1;transform:translateX(-50%) translateY(0)}\n::selection{background:rgba(84,64,246,.16);color:#0f172a}\n::-webkit-scrollbar{width:10px;height:10px}\n::-webkit-scrollbar-thumb{background:#d8dee8;border:3px solid transparent;border-radius:999px;background-clip:padding-box}\n@media(max-width:560px){.app{padding:16px 12px 56px}.card{padding:16px}.stat .v{font-size:22px}}'
    html = '<!doctype html><html lang="ru"><head><meta charset="utf-8">\n<meta name="viewport" content="width=device-width,initial-scale=1">\n<title>{{TITLE}}</title>\n<link rel="preconnect" href="https://fonts.googleapis.com">\n<link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>\n<link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800&display=swap" rel="stylesheet">\n<style>:root{--bg:#f6f8fb;--surface:#fff;--surface2:#f8fafc;--border:#eef0f4;--border2:#e2e8f0;\n--primary:#5440f6;--primaryD:#4532db;--primary50:#eef2ff;--primary100:#e0e7ff;\n--text:#0f172a;--muted:#64748b;--muted2:#94a3b8;--success:#16a34a;--successBg:#e7f8ed;\n--danger:#ef4444;--dangerBg:#fef2f2;--warnBg:#fef3c7;--warn:#b45309;\n--radius:14px;--radiusLg:18px;--radiusSm:10px;\n--shadow:0 1px 2px rgba(16,24,40,.04),0 10px 30px rgba(16,24,40,.06);\n--ring:0 0 0 3px rgba(84,64,246,.12),0 0 0 1px rgba(84,64,246,.36)}\n*{box-sizing:border-box}\nbody{margin:0;background:var(--bg);color:var(--text);\nfont-family:Inter,system-ui,-apple-system,"Segoe UI",Roboto,"Helvetica Neue",Arial,sans-serif;\n-webkit-font-smoothing:antialiased;text-rendering:geometricPrecision}\n.app{max-width:980px;margin:0 auto;padding:24px 16px 72px}\n.topbar{display:flex;align-items:center;gap:12px;margin-bottom:20px}\n.brand{display:flex;align-items:center;gap:10px;font-weight:800;font-size:19px;letter-spacing:-.01em}\n.brand .logo{width:32px;height:32px;border-radius:10px;background:linear-gradient(135deg,#5440f6,#8b7bff);\ndisplay:grid;place-items:center;color:#fff;font-weight:800;box-shadow:0 4px 12px rgba(84,64,246,.35)}\n.sub{color:var(--muted);font-size:13px;margin-top:2px}\n.card{background:var(--surface);border:1px solid var(--border);border-radius:var(--radiusLg);\nbox-shadow:var(--shadow);padding:22px;margin-bottom:16px}\n.card h2{margin:0 0 4px;font-size:16px;font-weight:700}\n.card .hint{color:var(--muted);font-size:13px;margin:0 0 16px}\n.btn{appearance:none;border:1px solid var(--border2);background:#fff;color:var(--text);font:inherit;\nfont-weight:600;padding:11px 18px;border-radius:12px;cursor:pointer;transition:.15s;display:inline-flex;\nalign-items:center;gap:8px}\n.btn:hover{background:var(--surface2)}\n.btn:active{transform:translateY(1px)}\n.btn-primary{background:var(--primary);border-color:var(--primary);color:#fff;\nbox-shadow:0 6px 16px rgba(84,64,246,.30)}\n.btn-primary:hover{background:var(--primaryD)}\n.btn:focus-visible{box-shadow:var(--ring)}\n.field{display:flex;flex-direction:column;gap:6px;margin-bottom:14px}\n.field label{font-size:13px;font-weight:600;color:var(--muted)}\n.input,select,textarea{font:inherit;padding:11px 13px;border:1px solid var(--border2);border-radius:12px;\nbackground:#fff;color:var(--text);width:100%;transition:.15s}\n.input:focus,select:focus,textarea:focus{outline:none;box-shadow:var(--ring);border-color:var(--primary)}\n.row{display:flex;gap:14px;flex-wrap:wrap}.row>.field{flex:1;min-width:170px}\ntable{width:100%;border-collapse:collapse;font-size:14px}\nthead th{text-align:left;font-weight:600;color:var(--muted2);padding:10px 12px;\nborder-bottom:1px solid var(--border);font-size:11px;text-transform:uppercase;letter-spacing:.04em}\ntbody td{padding:13px 12px;border-bottom:1px solid var(--border)}\ntbody tr:last-child td{border-bottom:0}\ntbody tr:hover td{background:var(--surface2)}\n.badge{display:inline-flex;align-items:center;gap:6px;padding:4px 11px;border-radius:999px;\nfont-size:12px;font-weight:600}\n.badge-success{background:var(--successBg);color:var(--success)}\n.badge-muted{background:var(--surface2);color:var(--muted)}\n.badge-warn{background:var(--warnBg);color:var(--warn)}\n.stat{display:flex;flex-direction:column;gap:4px}\n.stat .v{font-size:26px;font-weight:800;letter-spacing:-.02em}\n.stat .l{color:var(--muted);font-size:13px}\n.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:14px}\n.muted{color:var(--muted)}.right{text-align:right}.mt{margin-top:14px}\n.toast{position:fixed;left:50%;bottom:22px;transform:translateX(-50%) translateY(10px);background:#0f172a;\ncolor:#fff;padding:12px 20px;border-radius:12px;box-shadow:var(--shadow);font-size:14px;opacity:0;\ntransition:.25s;pointer-events:none;z-index:50}\n.toast.show{opacity:1;transform:translateX(-50%) translateY(0)}\n::selection{background:rgba(84,64,246,.16);color:#0f172a}\n::-webkit-scrollbar{width:10px;height:10px}\n::-webkit-scrollbar-thumb{background:#d8dee8;border:3px solid transparent;border-radius:999px;background-clip:padding-box}\n@media(max-width:560px){.app{padding:16px 12px 56px}.card{padding:16px}.stat .v{font-size:22px}}</style></head>\n<body>\n<div class="app">\n  <div class="topbar">\n    <div class="brand"><span class="logo">А</span>\n      <div>{{TITLE}}<div class="sub">Albery</div></div>\n    </div>\n  </div>\n  {{CONTENT}}\n</div>\n<div class="toast" id="toast"></div>\n{{APPLET}}\n<script>\nfunction toast(m){var t=document.getElementById(\'toast\');t.textContent=m;t.classList.add(\'show\');\nsetTimeout(function(){t.classList.remove(\'show\')},2200);}\n</script>\n</body></html>'
    content_example = '<div class="card">\n    <h2>Добавить запись</h2>\n    <p class="hint">Заполни поля и нажми «Сохранить».</p>\n    <div class="row">\n      <div class="field"><label>Название</label><input class="input" id="f_name" placeholder="Напр. Гастроскоп"></div>\n      <div class="field"><label>Сумма, ₽</label><input class="input" id="f_sum" type="number" placeholder="0"></div>\n    </div>\n    <button class="btn btn-primary" onclick="save()">Сохранить</button>\n  </div>\n  <div class="card">\n    <h2>Записи</h2>\n    <table><thead><tr><th>Название</th><th class="right">Сумма, ₽</th></tr></thead>\n    <tbody id="rows"><tr><td class="muted" colspan="2">Загрузка…</td></tr></tbody></table>\n  </div>\n  <script>\n  async function render(){var rows=await appletRows();var b=document.getElementById(\'rows\');\n    b.innerHTML=rows.slice(1).map(function(r){return \'<tr><td>\'+(r[0]||\'\')+\'</td><td class="right">\'+(r[1]||\'\')+\'</td></tr>\'}).join(\'\')||\'<tr><td class="muted" colspan="2">Пока пусто</td></tr>\';}\n  async function save(){var n=document.getElementById(\'f_name\').value,s=document.getElementById(\'f_sum\').value;\n    if(!n)return toast(\'Заполни название\');await appletAdd([n,s]);document.getElementById(\'f_name\').value=\'\';document.getElementById(\'f_sum\').value=\'\';toast(\'Сохранено ✓\');render();}\n  render();\n  </script>'
    if title:
        html = html.replace("{{TITLE}}", str(title))
    return {
        "css": css, "html_skeleton": html, "content_example": content_example,
        "palette": {"bg": "#f6f8fb", "primary": "#5440F6", "primaryDark": "#4532db",
                    "border": "#eef0f4", "text": "#0f172a", "muted": "#64748b",
                    "success": "#16a34a", "danger": "#ef4444", "radius": "14-18px", "font": "Inter"},
        "how_to": "1) make_sheet_applet(spreadsheet_id) -> applet_url+html_snippet. 2) get this template. "
                  "3) Replace {{TITLE}}; put your .card blocks (table/form/.stat) into {{CONTENT}} using "
                  "content_example as a guide; put the applet html_snippet into {{APPLET}}; read/write via "
                  "appletRows()/appletAdd([...]). 4) publish_web_app with doGet returning ONLY this HTML "
                  "(no SpreadsheetApp) so it opens anonymously.",
    }
...
def format_zoom_dispatch_summary(call: dict[str, Any]) -> str:
    ...
    Falls back to the ai_report top level for older payloads; empty string if the
...
def resolve_zoom_recipient(
    name: str,
    bitrix_user_id: int | None,
    team: list[dict[str, Any]],
    name_aliases: dict[str, str],
) -> dict[str, Any] | None:
    """Match a Zoom name / bitrix_user_id to one active org-structure member.

    Honours the editable alias instruction (e.g. "Анастасия Докучаева" ->
    "Анастасия Андрусяк"). Returns {name, user_id} or None if no confident match.
    """
    if bitrix_user_id is not None:
        for member in team:
            if to_int(member.get("user_id")) == bitrix_user_id:
                return {"name": str(member.get("name") or name).strip(), "user_id": to_int(member.get("user_id"))}
    match_name = name_aliases.get(normalize_person_name(name), name)
    if match_name and match_name != "Требует назначения":
        for member in team:
            ...
            if member_id is not None and full_name and person_names_match(full_name, match_name):
                return {"name": full_name, "user_id": member_id}
    return None


def find_zoom_leader_evaluation(
    recipient_name: str,
    bitrix_user_id: int | None,
    ...
    for ev in leader_evals:
        ev_id = to_int(ev.get("bitrix_user_id"))
        if ev_id is not None and bitrix_user_id is not None and ev_id == bitrix_user_id:
            return ev
...
def build_zoom_card_description(
    ...
    """Description of one person's "Итоги созвона" task: summary, then (for leaders)
    the evaluation, then their task list. No task list => заглушка (summary + eval only)."""
    parts: list[str] = []
...
def build_zoom_operational_tasks_dispatch(call_id: str, require_webhook: bool = False) -> dict[str, Any]:
    ...
    # Drop cards with nothing to deliver (no summary, no evaluation, no tasks) — e.g.
    # older reports without dispatch_summary for a participant who has no tasks.
    task_cards = [card for card in task_cards if str(card.get("description") or "").strip()]
    ...
    if not recipients:
        raise ValueError(
            "Не удалось собрать ни одного получателя по созвону: нет сопоставленных участников/задач "
            "или в отчёте отсутствуют выжимка и оценки."
        )
    description = (
        f"{ZOOM_OPERATIONAL_TASKS_DISPATCH_INTRO}\n\n"
        "Выделенные задачи с дедлайнами:\n"
        f"{cleaned_section}"
    ).strip() if cleaned_section else dispatch_summary
    return {
        "call": call,
        ...
        "leader_evaluations": leader_evals,
...
def dispatch_zoom_operational_tasks(call_id: str) -> dict[str, Any]:
    ...
    merged_skipped = list(existing_skipped)
    for name in payload.get("unmatched_assignees") or []:
        if name not in merged_skipped:
...
# Руководители для недельного свода собственнику (оценка ведения созвонов).
LEADER_DIGEST_NAMES = ["Артур Степанян", "Наталья Горюнова", "Евгений Палей", "Сергей Виноградов"]


def leader_evaluations_window() -> tuple[date, date]:
    """Default window for the Wednesday owner digest: previous Friday .. today.

    Run on Wednesday this captures Fri (prev week) + Mon + Tue + Wed — the week's
    leadership calls (today-5 lands on Friday when today is Wednesday).
    """
    today = msk_today()
    return today - timedelta(days=5), today


def collect_leader_evaluations_for_period(date_from: date, date_to: date) -> dict[str, Any]:
    """Aggregate saved leader_evaluations across zoom calls in [date_from, date_to]."""
    if not postgres_enabled():
        return {"date_from": date_from.isoformat(), "date_to": date_to.isoformat(), "calls_count": 0, "by_leader": []}
    ensure_zoom_schema()
    with pg_connect() as conn:
        with conn.cursor() as cur:
            cur.execute(
                """
                SELECT id, call_date, topic, technical_topic, start_time_msk,
                       raw_json->'ai_report'->'analysis'->'leader_evaluations' AS evals
                FROM zoom_calls
                WHERE call_date BETWEEN %s AND %s
                  AND jsonb_typeof(raw_json->'ai_report'->'analysis'->'leader_evaluations') = 'array'
    ...
    for row in rows:
        ...
        if not isinstance(evals, list) or not evals:
            continue
        calls_count += 1
        topic = str(row.get("topic") or row.get("technical_topic") or "").strip()
        call_date = row.get("call_date")
        call_date_iso = call_date.isoformat() if hasattr(call_date, "isoformat") else str(call_date)
        for ev in evals:
            ...
            canonical = name_aliases.get(normalize_person_name(name), name)
            key = f"user:{bitrix_user_id}" if bitrix_user_id is not None else f"name:{normalize_person_name(canonical)}"
            ...
            if entry is None:
                entry = {
                    ...
                    "items": [],
                    "counts": {"good": 0, "minor_issue": 0, "issue": 0},
                }
                by_key[key] = entry
                order.append(key)
            verdict = str(ev.get("verdict") or "").strip().lower()
            if verdict in entry["counts"]:
            ...
            entry["items"].append({
                ...
                "verdict": verdict,
                "result_for_owner": str(ev.get("result_for_owner") or "").strip(),
            })
    return {
        "date_from": date_from.isoformat(),
        "date_to": date_to.isoformat(),
        "calls_count": calls_count,
        "by_leader": [by_key[key] for key in order],
    }


def list_leader_evaluations(date_from: date | None = None, date_to: date | None = None) -> dict[str, Any]:
    """Read aggregated leader evaluations for the owner digest (default: prev Fri .. today)."""
    if date_from is None or date_to is None:
        default_from, default_to = leader_evaluations_window()
        date_from = date_from or default_from
        date_to = date_to or default_to
    if date_from > date_to:
        date_from, date_to = date_to, date_from
    return collect_leader_evaluations_for_period(date_from, date_to)


def _resolve_owner_recipient() -> dict[str, Any]:
    for member in load_team_members():
        if person_names_match(member.get("name"), "Евгений Палей"):
            user_id = to_int(member.get("user_id"))
            if user_id is not None:
                return {"name": str(member.get("name")).strip(), "user_id": user_id}
    raise ValueError("Не найден собственник «Евгений Палей» в активной оргструктуре.")


def dispatch_leader_evaluations_digest(
    digest_text: str,
    date_from: date | None = None,
    date_to: date | None = None,
) -> dict[str, Any]:
    """Create the weekly leader-evaluation digest as one Bitrix task for the owner.

    Deadline = next calendar day 10:00 МСК (digest sent Wed evening, owner reviews by
    Thursday 10:00). digest_text is supplied by the agent after owner approval.
    """
    if not os.getenv("BITRIX_WEBHOOK_BASE", "").strip():
        raise ValueError("Укажите BITRIX_WEBHOOK_BASE в файле .env.")
    digest_text = str(digest_text or "").strip()
    if not digest_text:
        raise ValueError("Пустой текст свода — нечего отправлять.")
    if date_from is None or date_to is None:
        default_from, default_to = leader_evaluations_window()
        date_from = date_from or default_from
        date_to = date_to or default_to
    ...
    title = f"Ознакомиться с оценкой руководителей за период {period_text}"
    deadline_at = datetime.combine(msk_today() + timedelta(days=1), dt_time(10, 0), tzinfo=MSK_TZ)
    ...
    return {
        ...
        "deadline": deadline_at.isoformat(),
        "deadline_text": f"{deadline_at.strftime('%d.%m.%Y')} 10:00 МСК",
        "date_from": date_from.isoformat(),
        "date_to": date_to.isoformat(),
    }
...
5. recommendation_feedback_context - обязательный источник тона и продолжения рекомендаций. Ответ сотрудника меняет формулировку новой рекомендации: благодарность за ответ, учет возражения, просьба подтвердить следующий шаг или мягкое повторение незакрытого вопроса.
6. company_regulations_context - обязательный источник проверки процесса. Если факт из Bitrix/чатов/Zoom расходится с регламентом, укажи это как причину рекомендации и предложи действие по регламенту.
...
- обратная связь по прошлой рекомендации показывает согласие, возражение, делегирование или странный ответ, и нужен следующий управленческий шаг;
- фактический исполнитель/владелец/срок/ритм работы не совпадает с регламентом компании.
...
- Начинай каждую рекомендацию с короткого мягкого вступления под конкретную обратную связь адресата:
  - если человек ответил по прошлой рекомендации: "Наталья, приветствую, благодарю за обратную связь. Учел(а) замечание по <тема>; давайте попробуем следующий шаг: ...";
  - если человек согласился/взял в работу: поблагодари и попроси конкретный следующий статус/срок;
  - если человек возразил или делегировал: признай аргумент и предложи вариант по регламенту/Bitrix;
...
- В каждой рекомендации обязательно сделай сверку с регламентом, если в company_regulations_context есть релевантное правило. Если факт расходится с регламентом, пиши прямо и мягко: "В регламенте по этой зоне владельцем указан(а) <ФИО/роль>, сейчас фактически ведет <ФИО>. Чтобы не держать процесс на тебе, можно передать/согласовать с <ФИО> и зафиксировать в Bitrix".
  ...
  "manager_messages": [
    {
      ...
      "message_text": "готовый текст для отправки в Bitrix от лица собственника, 2-6 строк; начинается с мягкого вступления с учетом обратной связи и содержит сверку с регламентом, если релевантно",
...
Перед заполнением пункта 6 и manager_messages обязательно прочитай recommendation_feedback_context. Каждое сообщение должно учитывать, что адресат ответил на прошлую рекомендацию: поблагодарить за ответ, признать замечание/возражение, продолжить новым конкретным шагом. Если ответа нет, начать нейтрально и попросить короткий статус.
Перед заполнением пункта 6 и manager_messages обязательно прочитай company_regulations_context. Каждая рекомендация должна быть сверена с регламентами компании. Если фактический владелец, исполнитель, срок, порядок согласования или ритм работы расходится с регламентом, укажи это в тексте рекомендации и предложи действие по регламенту: делегировать правильному владельцу, согласовать исключение, зафиксировать изменение в Bitrix или обновить регламент.
...
- message_type;
- subject;
- message_text: тот же готовый текст, что в пункте 6; первая фраза должна быть мягким вступлением с учетом обратной связи адресата, а тело должно содержать регламентную сверку, если релевантное правило найдено;
- due;
- topics;
- source_facts.
...
def _compact_owner_recommendation_feedback_for_prompt(report_date: date, limit: int = 80) -> dict[str, Any]:
    if not postgres_enabled():
        return {"period_checked": None, "items": []}
    ensure_owner_reports_schema()
    previous_date = report_date - timedelta(days=1)
    with pg_connect() as conn:
        with conn.cursor() as cur:
            cur.execute(
                """
                SELECT
                    r.id, r.status, r.priority, r.subject, r.recommendation_text,
                    r.expected_action, r.due_date, r.response_due_at,
                    r.sent_at, r.last_response_at, r.manager_review_required,
                    r.current_interpretation,
                    mu.full_name AS manager_name,
                    mu.bitrix_user_id AS manager_bitrix_user_id,
                    eu.full_name AS employee_name,
                    eu.bitrix_user_id AS employee_bitrix_user_id,
            ...
            events_by_recommendation: dict[str, list[dict[str, Any]]] = {}
            if recommendations:
                ...
                cur.execute(
                    ...
                    SELECT
                        ...
                        e.dialog_id, c.chat_title, e.chat_message_day,
                        e.old_status, e.new_status, e.event_text,
                        e.interpretation, e.event_at
                    FROM owner_recommendation_events e
                    LEFT JOIN users u ON u.id = e.author_user_id
                    LEFT JOIN chats c ON c.id = e.chat_id
                    WHERE e.recommendation_id = ANY(%s::uuid[])
                      AND (
                          e.chat_message_day BETWEEN %s AND %s
                          OR e.event_at::date BETWEEN %s AND %s
                      )
                    ORDER BY e.event_at DESC
                    LIMIT 240
                    """,
                    (recommendation_ids, previous_date, report_date, previous_date, report_date),
                )
                for event in cur.fetchall():
                    key = str(event["recommendation_id"])
                    events_by_recommendation.setdefault(key, []).append({
                        "event_type": event.get("event_type"),
                        "author_type": event.get("author_type"),
                        "author_name": event.get("author_name"),
                        "author_bitrix_user_id": event.get("author_bitrix_user_id"),
                        "dialog_id": event.get("dialog_id"),
                        "chat_title": event.get("chat_title"),
                        "chat_message_day": event.get("chat_message_day").isoformat() if hasattr(event.get("chat_message_day"), "isoformat") else event.get("chat_message_day"),
                        "old_status": event.get("old_status"),
                        "new_status": event.get("new_status"),
                        "event_text": str(event.get("event_text") or "")[:1200],
                        "interpretation": event.get("interpretation"),
                        "event_at": event.get("event_at").isoformat() if hasattr(event.get("event_at"), "isoformat") else str(event.get("event_at") or ""),
                    })
    ...
    for row in recommendations:
        ...
        items.append({
            ...
            "recommendation_text": str(row.get("recommendation_text") or "")[:2000],
            "expected_action": str(row.get("expected_action") or "")[:1200],
            "due_date": row.get("due_date").isoformat() if hasattr(row.get("due_date"), "isoformat") else row.get("due_date"),
            "response_due_at": row.get("response_due_at").isoformat() if hasattr(row.get("response_due_at"), "isoformat") else row.get("response_due_at"),
            "sent_at": row.get("sent_at").isoformat() if hasattr(row.get("sent_at"), "isoformat") else row.get("sent_at"),
            "last_response_at": row.get("last_response_at").isoformat() if hasattr(row.get("last_response_at"), "isoformat") else row.get("last_response_at"),
            "manager_review_required": bool(row.get("manager_review_required")),
            "current_interpretation": row.get("current_interpretation"),
            "manager_name": row.get("manager_name"),
            "manager_bitrix_user_id": row.get("manager_bitrix_user_id"),
            "employee_name": row.get("employee_name"),
            "employee_bitrix_user_id": row.get("employee_bitrix_user_id"),
            "feedback_dialog_id": row.get("feedback_dialog_id"),
            "feedback_chat_title": row.get("feedback_chat_title"),
            "events": events_by_recommendation.get(recommendation_id, []),
        })
    return {
        "period_checked": {"date_from": previous_date.isoformat(), "date_to": report_date.isoformat()},
        "items": items,
        "rule": (
            "Use this block to adapt the opening and next step of every addressable recommendation. "
            "Thank people for concrete feedback, account for objections/delegation, and ask for a precise next status/date. "
            "No response means the recommendation remains open unless other current sources explicitly close it."
        ),
    }
            ...
            cur.execute(
                ...
                WHERE c.is_excluded = FALSE
                  ...
                  AND EXISTS (
                      ...
                      WHERE mx.chat_id = c.id AND mx.message_day = %s
                  )
...
- Если в чате за дату нет сообщений, отчет не формируется вообще: не создавай заглушку, не создавай `no_data`, не анализируй хвосты и не вызывай модель для этого чата.
...
def generate_ai_chat_report(dialog_id: str, target_date: date, force: bool = False) -> dict[str, Any]:
    ...
    if target_date > REPORT_PLACEHOLDER_START_DATE:
        previous_date = target_date - timedelta(days=1)
        previous_report = load_chat_day(dialog_id, previous_date).get("report")
        if chat_day_has_messages(dialog_id, previous_date) and not is_completed_chat_day_report(previous_report):
            raise ValueError(
    ...
    if not (chat_day.get("messages") or []):
        return chat_day | {
            "skipped": True,
            "message": "В чате нет сообщений за дату; ежедневный отчет не создан.",
        }
...
AUTH_EXEMPT_ROUTES = frozenset({
    "/login",
    "/logout",
    # Проверка живости для деплоя и мониторинга: доступна без входа намеренно, иначе её не
    # выполнить из скрипта — а именно её отсутствие 07.08.2026 позволило выложить службы со
...
# Bitrix24 chat-bot + agent-access API (moved out verbatim 2026-07-02). Imported at the
# bottom so every shared helper above is already defined; the module registers its
# @app routes on import.
...
from zoom import (  # noqa: E402
    ...
    _ZOOM_PARTICIPANT_ARTIFACTS,
    _zoom_ai_analysis,
    _zoom_export_filename,
    _zoom_export_link_payload,
    _zoom_export_public_url,
    _zoom_md_clean_offset,
    _zoom_person_summaries,
    _zoom_report_people_for_participant_reports,
    ...
    process_zoom_recording_event_queue,
    upsert_zoom_recording_meeting,
    zoom_call_leader_evaluations,
    zoom_call_operational_tasks,
    zoom_call_participants,
    zoom_call_row_payload,
    write_export_display_name,
    zoom_credentials_status,
    zoom_dispatch_deadline,
    zoom_dispatch_title,
    zoom_event_secret_valid,
    zoom_list_recordings,
    zoom_list_users,
    zoom_session,
)

# Google Drive integration (moved out verbatim 2026-07-02): registers routes on import; the
import gdrive  # noqa: E402,F401
from gdrive import (  # noqa: E402
    _extract_drive_folder_id,
    _google_user_credentials,
    ensure_company_profile_schema,
    google_drive_company_sync_config,
    load_zoom_calls_tree,
    record_integration_sync_success,
    resolve_zoom_participants,
)

# Bitrix24 integration (moved out verbatim 2026-07-02): registers webhook routes on
# import; the from-import puts the names the remaining app.py code still calls back
# into this namespace (resolved at call time, so the bottom-of-file position is safe).
import bitrix  # noqa: E402,F401
from bitrix import (  # noqa: E402
    BitrixClient,
    bitrix_event_secret_valid,
    bitrix_origin_from_webhook,
    bitrix_storage_id_for_reports,
    bitrix_webhook_client,
    build_task_record,
    delete_task_records,
    extract_chat_id,
    extract_files,
    extract_task_created_date,
    extract_task_id,
    load_team_members,
    pg_user_id_by_bitrix,
    process_bitrix_task_event_queue,
    resolve_person,
    sync_bitrix_team,
    to_list,
    upsert_task_records,
