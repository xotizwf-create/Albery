...
Design constraints (owner: «надстройка небольшая, аккуратно и бесшовно»):
  - the offer text is generated by the tool-free Codex quality contour; media-only Groq is not
    allowed to make employee-facing decisions; on any Codex failure a fixed honest fallback is used;
...
def compose_offer(task: dict[str, Any], candidates: list[dict[str, Any]],
    ...
    Engine chain: isolated Codex quality turn → fixed honest fallback. Groq is intentionally
    media-only and never writes employee-facing recommendations."""
...
def _post_offer(task_id: int, title: str, description: str, checklist: Any,
    ...
    if not message.strip():
        logging.info("task offers: no useful offer for task %s", task_id)
        return
