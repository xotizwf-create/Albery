...
def _b24_native_artifacts_enabled() -> bool:
    return os.getenv("CHANNEL_NATIVE_ARTIFACTS", "1").strip().lower() not in {"0", "false", "no", "off"}


def _b24_app_file_reply(client_endpoint: str, access_token: str, bot_id: Any, dialog_id: str,
                        text: str, artifacts: list[dict[str, Any]]) -> tuple[Any, str | None]:
    """Upload exact generated bytes into the bot dialog through Bitrix' native file API.

    Returns ``(last_message_id, error_kind)`` where error_kind is ``known`` (the portal rejected
    the call and a plain explanatory message is safe) or ``ambiguous`` (the connection failed after
    send may have started, so automatic replay/fallback could duplicate the file).
    """
    max_bytes = int(os.getenv("B24_NATIVE_ARTIFACT_MAX_BYTES", str(40 * 1024 * 1024)) or 40 * 1024 * 1024)
    rendered = bb_sanitize(text)
    disclaimer = _b24_disclaimer()
    if disclaimer:
        rendered = (rendered + f"\n\n[i]{disclaimer}[/i]").strip()
    last_id: Any = None
    for index, artifact in enumerate(artifacts):
        data = artifact.get("data") or b""
        name = os.path.basename(str(artifact.get("display_name") or artifact.get("filename") or "file"))
        if not data or len(data) > max_bytes:
            logging.error("b24 native artifact rejected locally: name=%s bytes=%d", name, len(data))
            return last_id, "known"
        caption = rendered if index == 0 else f"📎 Дополнительный файл: {name}"
        payload = {
            "botId": int(bot_id),
        ...
        try:
            response = _b24_app_call(
                client_endpoint, access_token, "imbot.v2.File.upload", payload, _log=False
            )
        except (requests.Timeout, requests.ConnectionError):
            logging.error("b24 native artifact outcome ambiguous: bot=%s dialog=%s", bot_id, dialog_id)
            return last_id, "ambiguous"
        except Exception:  # noqa: BLE001
            logging.exception("b24 native artifact upload rejected: bot=%s dialog=%s", bot_id, dialog_id)
            return last_id, "known"
        ...
        log_bot_message(
            ...
            meta={"native_artifact": True, "file_name": name, "byte_size": len(data)},
        )
    return last_id, None
    ...
    try:
        ...
        for m in EXPORT_HANDOFF_RE.finditer(answer or ""):
            ...
            _att.store_attachment(
                data=data, file_name=export_display_name(fname), kind="agent_doc",
