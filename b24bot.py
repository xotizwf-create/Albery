...
def _b24_app_register_openline_bot(client_endpoint: str, access_token: str) -> Any:
    """Бот ОТКРЫТОЙ ЛИНИИ — отдельный от бота сотрудников.

    TYPE='O' + OPENLINE='Y' — только такой бот выбирается в настройках линии (проверено по
    документации 26.07.2026). Бот сотрудников (`hermes_agent`, TYPE='B') для линии не годится, и
    смешивать их нельзя: у клиента снаружи и у сотрудника внутри разные правила и разный доступ."""
    payload = {
        "CODE": "albery_iu_openline",
        "TYPE": "O",
        "OPENLINE": "Y",
        "EVENT_MESSAGE_ADD": B24_APP_HANDLER_URL,
        "EVENT_WELCOME_MESSAGE": B24_APP_HANDLER_URL,
        "EVENT_BOT_DELETE": B24_APP_HANDLER_URL,
        "PROPERTIES": {"NAME": "Албери — консультант ИУ", "COLOR": "GREEN",
                       "WORK_POSITION": "Индивидуальные условия Wildberries"},
    }
    return _b24_app_call(client_endpoint, access_token, "imbot.register", payload).get("result")
...
class _OpenlineBitrix:
    """Портал глазами канала: один метод `call`, чтобы ход целиком проверялся тестами."""
    ...
    def call(self, method: str, params: dict[str, Any] | None = None) -> dict[str, Any]:
        return _b24_app_call(self.endpoint, self.access_token, method, params or {}, _log=False)


def _openline_brain(dialog, text: str, history: str) -> str:
    """Ответ клиенту в открытой линии.

    Мозг берём тот же, что ведёт лидов в Telegram: карточка агента (её владелец правит сам, без
    деплоя) + ШАГ ВОРОНКИ, считанный из сделки прямо сейчас, + история этого диалога. Импорт
    ленивый: веб-приложение не должно тянуть Telegram-службу, пока в линию никто не написал."""
    import tg_agent

    role = tg_agent.channel_role_prompt(tg_agent.MANAGER_CHANNEL)
    step = tg_agent.funnel_step_block(dialog.deal_id) if dialog.deal_id else ""
    parts = [
        role or (
            "Ты ведёшь переписку от лица компании Albery с клиентом по партнёрской программе "
            "Wildberries (индивидуальные условия). Отвечай по-русски, коротко и по-человечески, "
            "обычным текстом без разметки — 1-3 предложения. Не представляйся ботом."
        ),
        tg_agent.STYLE_RULES,
        f"Сделка в CRM: №{dialog.deal_id} (воронка «Партнёрская программа WB — индивидуальные условия»)",
        step,
    ]
    if history:
        parts.append("История этого разговора (помни её, не здоровайся повторно):\n" + history)
    parts.append(f"Сообщение клиента:\n{text}")
    prompt = "\n\n".join(p for p in parts if p)
    return tg_agent.customer_hermes_answer(prompt, f"ol-{dialog.chat_id}")


def _openline_journal(*, dialog, direction: str, text: str, author: str, event) -> None:
    """Переписка линии — в общий журнал бот-сообщений. В карточку сделки её кладёт сам Битрикс."""
    log_bot_message(direction=direction, dialog_id=dialog.dialog_id, text=text,
                    bot_id=event.bot_id, bitrix_user_id=event.author_id, kind="openline",
                    meta={"deal_id": dialog.deal_id, "author": author, "line_id": dialog.line_id})


def _openline_event(payload: dict[str, Any], event_name: str, endpoint: str,
                    access_token: str, bot_id: str):
    """Событие бота открытой линии. Ход мозга уходит в поток: Битрикс ждать не должен."""
    import openline_agent as ol

    if not ol.enabled():
        return jsonify({"ok": True, "event": event_name, "openline": "disabled"})
    if event_name != "ONIMBOTMESSAGEADD":
        return jsonify({"ok": True, "event": event_name, "openline": "skipped"})

    dialog_id = str(_imbot_event_param(payload, "DIALOG_ID") or "")
    chat_id = to_int(dialog_id[4:]) if dialog_id.lower().startswith("chat") else None
    message_id = _imbot_event_param(payload, "MESSAGE_ID") or ""
    if chat_id is None:
        return jsonify({"ok": True, "event": event_name, "openline": "not-a-line-chat"})
    # Повторная доставка события не должна отвечать клиенту дважды — та же защита, что у бота
    # сотрудников, и она обязана стоять ДО любого действия.
    if message_id and not _b24_message_claim(message_id, bot_id, dialog_id,
                                             _imbot_event_param(payload, "FROM_USER_ID")):
        return jsonify({"ok": True, "event": event_name, "duplicate": True})

    event = ol.Event(
        chat_id=chat_id,
        author_id=_imbot_event_param(payload, "FROM_USER_ID"),
        text=str(_imbot_event_param(payload, "MESSAGE") or "").strip(),
    ...
    def _run() -> None:
        try:
            state = _b24_load_state()
            bot_ids = {str(bot_id), str(state.get("bot_id") or ""), _openline_bot_id(state)}
            ol.handle_event(
                event,
                bitrix=_OpenlineBitrix(endpoint, access_token),
                db=pg_connect,
                brain=_openline_brain,
                journal=_openline_journal,
                bot_ids={b for b in bot_ids if b},
                funnel_category_id=int(os.getenv("CRM_LEAD_CATEGORY_ID", "16") or 16),
                agent_enabled=_openline_agent_enabled,
            )
        except Exception:  # noqa: BLE001
            logging.exception("openline: ход по диалогу %s не выполнен", chat_id)

    threading.Thread(target=_run, daemon=True, name=f"openline-{chat_id}").start()
    return jsonify({"ok": True, "event": event_name, "openline": "accepted"})


def _openline_agent_enabled() -> bool:
    """Выключатель воронки из кабинета («Работа с воронками») — он главнее кода и канала."""
    import funnel_scenario

    from shared.db import connect
    return bool(funnel_scenario.agent_enabled(
        connect, int(os.getenv("CRM_LEAD_CATEGORY_ID", "16") or 16)))
...
def hermes_brain_answer(user_text: str, dialog_id: str, tier: str = "faq", from_user_id: Any = "",
    ...
    if tier in ("ops", "admin") and str(from_user_id).strip():
        parts.append(
            ...
            "ПРАВКА ПРИСЛАННОГО ФАЙЛА (важно): если пользователь просит изменить/исправить/дополнить "
            "документ, который он прислал, — вызывай edit_attachment_document(attachment_id, "
            "edits=[{find, replace}]): меняются ТОЛЬКО указанные места, всё остальное (таблицы, ссылки, "
            "оформление) сохраняется из оригинала; пользователю отправь ссылку из ответа инструмента. "
            "Точный find бери из get_attachment_text. Пересобирать файл заново (export_document или "
            "маркер файла) при просьбе о правке НЕЛЬЗЯ — потеряешь содержимое, которое не пересказал. "
            "export_document — только для НОВОГО документа с нуля."
        )
    if seed:
    ...
    if run_fail == "timeout":
        ...
        if sheets_like:
            return (
                ("[b]Не успел за %s сек. — остановил этот ход.[/b]\n\n" % timeout_s)
                + "Либо операция с таблицами слишком объёмная для одного сообщения, либо это разовый "
                + "сбой канала к модели. Я не буду писать, что файл готов, пока он реально не создан.\n\n"
                + "Отправьте запрос ещё раз — чаще всего со второй попытки получается. Если снова не "
                + "успею — разбейте задачу: сначала 1-2 артикула, проверить, потом остальное."
            )
        return (
            ("[b]Не успел за %s сек. — остановил этот ход.[/b]\n\n" % timeout_s)
            + "Чаще всего это разовый сбой — просто отправьте сообщение ещё раз. Если задача "
            + "действительно большая, давайте разобьём её на короткие шаги."
        )
...
def _bitrix_imbot_app_event():
    ...
    # Открытая линия: у бота линии свой BOT_ID и совсем другие правила — снаружи КЛИЕНТ, а не
    # сотрудник, и разговор в любой момент забирает себе человек. Уводим такие события целиком
    # в openline_agent, не смешивая с ботом сотрудников.
    openline_bot = _openline_bot_id(state)
    if openline_bot and event_bot_id == openline_bot:
        return _openline_event(payload, event_name, endpoint, access_token, event_bot_id)
