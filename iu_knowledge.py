...
Формат блока в документе владельца (разделитель — строка `---`):
    ...
    ### Комиссия
    Спрашивают так: сколько берёте, ваша комиссия, процент с продаж
...
Строка «Спрашивают так» — это формулировки КЛИЕНТА, а не заголовок темы: клиенты спрашивают
«сколько вы берёте», а не «комиссия». Без неё карточка не находится, и агент снова эскалирует.
...
def human_required(found) -> str:
    """Условие владельца «здесь всегда нужен человек», если оно стоит на найденной карточке."""
    for hit in found:
