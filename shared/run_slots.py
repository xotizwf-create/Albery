...
def build_default() -> RunSlots:
    ...
    Оба поднимают процессы `hermes` на одной и той же коробке с 2 ГБ, но до 06.08.2026
    считали их независимо: b24bot разрешал свои 3 и tg_agent (отдельная служба albery-tg) —
    свои 3. То есть фактический потолок был 6 ходов, а не заявленные 3. Лимит принадлежит
    ЖЕЛЕЗУ, а не отдельной службе, поэтому пул здесь общий; TG_AGENT_PARALLEL_TURNS больше
    не заводит второй счётчик.

    Умолчание 2, а не 3 (изменено 07.08.2026 по замеру на проде). Ход мозга занимает НЕ
    250 МБ, как считалось раньше, а до 396 МБ — столько взял живой ход обработки зум-созвона
    с транскриптом. Арифметика коробки на 2 ГБ:
        службы Albery ~305 МБ + hermes-gateway ~303 + PostgreSQL ~260 = ~870 МБ занято,
        свободно при спокойной работе ~890 МБ;
        три хода = 1188 МБ → своп и риск OOM;
        два хода =  792 МБ → остаётся ~100 МБ запаса.
    Убийств по памяти пока не было только потому, что нагрузка редкая: пик 10 ходов в час,
    обычно 1–4, и три одновременных ни разу не совпали. Это везение, а не запас.
    Бюджет считается по ТЯЖЁЛОМУ ходу, а не по среднему — именно на среднем я и ошибся.
    """
    global _default
    with _default_lock:
        if _default is None:
            limit = max(1, int(os.getenv("B24_HERMES_MAX_CONCURRENCY", "2") or "2"))
