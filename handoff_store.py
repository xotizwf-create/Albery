...
def overdue_handoffs(
    ...
    *,
    ...
    """Return overdue open items eligible for another owner reminder."""
    ...
    with db() as conn:
        with conn.cursor() as cur:
            cur.execute(
                """
                SELECT id, bot, dialog_id, deal_id, status, priority, reason_code,
                       owner_id, owner_name, due_at, destination, external_message_id,
                       customer_notified, event_count
                  FROM ai_handoffs
