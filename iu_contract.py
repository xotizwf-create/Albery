...
#: Вопрос о самом собеседнике: «вы бот?», «это ИИ?», «как вас зовут?», «вы живой человек?».
#: Это знакомство, а не запрос коммерческого факта.
_IDENTITY_RE = re.compile(
    r"(?<![а-яё])(?:ты|вы)\s+(?:бот|робот|живой|настоящ\w*|реальн\w*)|"
    r"(?<![а-яё])(?:ты|вы)\s+(?:—|-)?\s*(?:ии|ai|нейросет\w*|бот)|"
    r"(?:это|у\s+вас)\s+(?:бот|ии|нейросет\w*|автоответчик)|"
    r"как\s+(?:вас|тебя)\s+зовут|"
    r"с\s+кем\s+я\s+(?:говорю|общаюсь|переписываюсь)|"
    r"человек\s+или\s+бот|бот\s+или\s+человек",
    re.I,
)


def is_identity_question(message: str) -> bool:
    """Спрашивает ли клиент, с кем он разговаривает."""
    return bool(_IDENTITY_RE.search(str(message or "")))
...
def states_facts(plan: TurnPlan, message: str = "") -> bool:
    ...
    text = plan.reply
    # Цифра в ответе — утверждение о деле при любом вопросе. Проверяем её первой, поэтому
    # знакомством нельзя прикрыть названную от себя ставку.
    if _NUMBER_RE.search(text):
        return True
    if is_identity_question(message):
        # «Вы бот или живой человек?» — карточки знаний на это нет и быть не может, поэтому
        # порог заваливал ответ и клиент получал «уточню это у команды» на простой вопрос
        # (живой прогон 29.07.2026). Достаточно, что коммерческая лексика в такой реплике
        # («работаю по вопросам подключения и условий») ничего не утверждает.
        return False
...
#:
#: Номер бывает и внутри жирного заголовка, и диапазоном: отвечая на список вопросов клиента,
#: агент пишет «[b]4–5. Доступ к карточкам и ценам[/b]» — это ссылка на пункты ЕГО письма.
#: Пробел после точки обязателен, иначе «1.5 млн» потеряло бы целую часть и превратилось в «5».
_LIST_ORDINAL_RE = re.compile(
    r"(?m)^\s*(?:\[b\]\s*)?\d{1,2}(?:\s*[–—−-]\s*\d{1,2})?[.)]\s+")
