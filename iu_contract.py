...
_CALC_RE = re.compile(
    ...
    # «Выплаты» намеренно НЕ здесь: «когда приходят выплаты» — вопрос о сроках процесса, а не
    # вычисление. Строгий порог на нём только уводил бы к людям обычный вопрос о регламенте.
    r"оборот\w*|доход\w*|окупаемост\w*|\bДРР\b|\bдрр\b|"
...
def is_calculation(plan: TurnPlan, message: str = "") -> bool:
    ...
    Ловим и по вопросу клиента, и по ответу модели: клиент может попросить «посчитайте мою
    экономику», а модель — сама начать объяснять, от чего считается процент.

    Цитата ставки («комиссия 44%») расчётом НЕ считается: новых чисел она не выводит, а её
    защищает вето на числа, которых нет в источниках."""
    haystack = f"{message}\n{plan.reply}"
    return bool(_CALC_RE.search(haystack))
