...
заполняет форму; лид и анкета соединяются в одну сделку».
Уточнение владельца 30.07.2026: подтверждённое заполнение анкеты — единственное
автоматическое событие воронки: карточка переходит из «Нового клиента» в «Анкету».
Дальше этап меняют только человек или явный инструмент ИИ.
...
#: Узкое правило владельца: успешная склейка анкеты переводит только NEW → «Анкета».
#: Отдельный аварийный тумблер позволяет остановить именно этот переход, не затрагивая
#: перенос полей и удаление дубля. Общие неявные переходы воронки остаются запрещены.
FORM_STAGE_TRANSITION_ENABLED = os.getenv(
    "IU_FORM_STAGE_TRANSITION_ENABLED",
    "1",
).strip().lower() in {"1", "true", "yes", "on"}

#: Стадия, на которую переезжает карточка после подтверждённой анкеты.
...
def next_stage_for(target_deal) -> str:
    """После анкеты двигать только «Новый клиент» → «Анкета».

    Карточку, которую человек или ИИ уже продвинули дальше, не трогаем: повторная либо
    запоздалая анкета не имеет права откатывать этап назад.
    """
    if not FORM_STAGE_TRANSITION_ENABLED:
        return ""
