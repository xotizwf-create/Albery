...
Уточнение владельца 30.07.2026: склейка переносит данные, но не меняет этап — воронкой
управляют только человек или явный инструмент ИИ.
...
#: Автоматические переходы отключены владельцем 30.07.2026. Поля анкеты по-прежнему
#: переносятся и дубль удаляется, но этап меняют только человек или явный инструмент ИИ.
AUTOMATIC_STAGE_TRANSITIONS_ENABLED = os.getenv(
    "IU_AUTOMATIC_STAGE_TRANSITIONS_ENABLED",
    "0",
).strip().lower() in {"1", "true", "yes", "on"}

#: Стадия, на которую раньше автоматически переезжала карточка после анкеты.
...
def next_stage_for(target_deal) -> str:
    """Куда двигать карточку. По умолчанию всегда пусто: автоматика отключена.

    Старый расчёт оставлен за аварийным env-тумблером, чтобы будущие явно утверждённые
    правила можно было включить контролируемо. Без него заполнение анкеты меняет данные,
    но никогда не принимает решение за менеджера или ИИ об этапе сделки.
    """
