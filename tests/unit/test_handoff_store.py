...
def test_open_event_is_atomic_idempotent_and_keeps_original_sla():
    ...
    handoff_sql, handoff_params = cur.executed[2]
    event_sql, event_params = cur.executed[3]
...
def test_duplicate_event_returns_existing_delivery_outcomes():
    ...
    assert "WHERE e.event_key = %s" in cur.executed[1][0]
...
def test_delivery_claim_is_a_compare_and_set():
    ...
    assert "customer_delivery_started_at = now()" in claimed.executed[0][0]
...
def test_overdue_query_has_owner_and_no_duplicated_client_text():
    ...
    assert "reminder_count = reminder_count + 1" in sql
    assert "all_customer_deliveries_sent" in sql
