...
- `fetch_url` — делает внешний HTTP GET и умеет переписывать ссылки Google Docs/Sheets в экспортный формат. БД не пишет и ничего не отправляет от имени компании, но это всё равно внешняя сеть и риск SSRF/утечки URL-контекста. Нужны allowlist/ограничения доменов или явное правило, где его можно использовать.
...
В MCP выглядит как обработка, не как запись, но фактически вызывает backend workflow `process_chat_image_ocr_for_period`. Это действие, которое читает вложения/файлы, прогоняет OCR и обновляет состояние OCR в БД. Сейчас confirm-gate в MCP отсутствует. Риск ниже, чем у отправки в Bitrix, но это не read-only.
...
2. `process_chat_ocr` выглядит как служебная обработка, но фактически меняет состояние OCR/БД. Сейчас без подтверждения.
...
4. `fetch_url` разрешает внешний HTTP GET по произвольному URL. Нужны ограничения или отдельный регламент использования.
...
- `external_read` — без подтверждения только для allowlist-доменов; иначе с явным разрешением.
