...
0 * * * * root cd $APP_DIR && ALBERY_LOG_DIR=$LOG_DIR ALBERY_DAILY_SYNC_LOG=$LOG_DIR/daily-sync.log $APP_DIR/.venv/bin/python $APP_DIR/scripts/run_daily_sync.py >> $LOG_FILE 2>&1
EOF

chmod 644 "$CRON_FILE"

echo "Installed hourly Albery sync cron: $CRON_FILE"
