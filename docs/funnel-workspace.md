...
- Любой новый свободный вопрос ночью не запускает ИИ, а передаётся менеджеру.
  Клиент один раз за ночной период получает:
  «Передал ваш запрос менеджеру. Сейчас нерабочее время — менеджер ответит
  завтра утром!»
- Повторное сообщение той же ночью не молчит: вместо дубликата ночного уведомления бот
  возвращает безопасную подсказку нажать «Задать вопрос» и главное меню. Повторный handoff
  и лишнее событие управления не создаются.
