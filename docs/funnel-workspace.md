...
- Повторное нажатие «Присоединиться к ИУ» при уже заполненной анкете означает повторный
  вход клиента в сценарий: диалог передаётся менеджеру и в группу уходит такое же
  уведомление о необходимости ответа.

Кнопки этого вопроса имеют отдельные события состояния и не пересекаются с
