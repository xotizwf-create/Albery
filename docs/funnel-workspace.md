...
сценария режиме `human`. Если клиент до этого явно вызвал менеджера, `/start`, `/menu` или
«Перейти в меню» отвечают: «Диалог с менеджером отменён. Вы вернулись в главное меню»,
снимают назначение и только после этого возвращают управление ИИ.
...
- Только после явного действия «Позвать оператора/менеджера» бот убирает клавиатуру,
  переводит диалог в бессрочный `human` и в нерабочее время сообщает:
  «Передал ваш запрос менеджеру. Сейчас нерабочее время — менеджер ответит
  завтра утром!»
- Автоматическая неуверенность/ошибка ИИ может поднять обращение в очередь менеджера,
  но не прекращает следующие ответы ИИ. Блокирующий handoff создаёт только клиент.
