...
- **Клавиатура** (`_b24_keyboard`) — 3 кнопки: «🆕 Новая сессия», «⚠️ Ошибка/Предложение»
  (до 2026-07-09 — «Сообщить об ошибке»; принимает и ошибки, и предложения по улучшениям),
...
- Кнопка «⚠️ Ошибка/Предложение» → бот просит описать ошибку или предложение → следующее
  сообщение БЕЗ вызова модели уходит в **Telegram-группу «Albery_Уведомления» (`-5283789593`)**
  в формате «{Имя} отправил ошибку/предложение: …» + лог в `bitrix_error_reports`. Доставка через Telegram Bot API
