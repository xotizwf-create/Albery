...
## Playbooks: фиксация успешных решений / Success Playbooks

**Мета-правило.** Когда мы пришли к успешному нетривиальному решению (новая возможность, починка повторяющейся проблемы, найденный рабочий подход) — **обязательно зафиксировать playbook**: краткую инструкцию «проблема → как решили → как делать в следующий раз», ЕСЛИ ещё нет смежного playbook на эту тему. Если смежный есть — дополнить его, а не плодить дубликаты.

Где хранить:
- Playbooks лежат в `docs/playbooks/<kebab-case>.md` (заголовок H1 — человекочитаемое русское название), в git.
- Каждый новый playbook добавляется ссылкой в индекс ниже.
- Если решение меняет поведение Hermes/коннектора в рантайме — отдельно обнови AI-инструкцию (Albery UI → «Настройки → Инструкции для ИИ» / `scripts/ai_instruction_*.md` через `scripts/upsert_albery_ai_instruction.py`); playbook описывает инженерную сторону, AI-инструкция — поведение ассистента.

Что должен содержать playbook: симптом/проблема, корневая причина, рабочее решение, конкретные файлы/функции/маршруты/env, чек-лист «как сделать так же в следующий раз», и как это проверено.

Индекс playbooks:
- [Передача файлов через МСП](docs/playbooks/file-delivery-via-mcp.md) — большие выгрузки через MCP отдаём ссылкой на скачивание (а не текстом, который режут клиенты); селекторы принимают явные списки (`dates`/`call_ids`), чтобы не зацикливаться на пагинации.
- [О проекте / about-project](docs/about-project.md) — обзор проекта (пока «зерно»); подробно задокументирована надстройка «Оценка руководителей по Zoom» (контракт `zoom_processing` v9, диспетчер по участникам с выжимкой/оценкой/заглушками, недельный свод собственнику по средам 19:00). Деплой 2026-06-02.
...
напрямую и `401` (т.е. дошли, нужен только ключ) через VPN. Codex/googleapis
...
```bash
hermes auth add openai --type api-key --api-key sk-...     # или anthropic / codex
hermes config set model.provider openai                    # anthropic | codex
